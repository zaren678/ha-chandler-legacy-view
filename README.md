...
* **Controls:** `Refresh Now` (dashboard `117` only), `Sync Time` (Evb019 only; uses Home Assistant local time and the official app's Dashboard command), and guarded `Regenerate Now` / `Next Regeneration Step` (Evb019 only, refreshes state before send so only valid action is available). Regen cycle timings are intentionally not writable.
* **Connectivity diagnostics:** `Last Successful Update` records each valid Dashboard response for watchdogs and troubleshooting; `Last Clock Sync` records successful Set Time writes.
