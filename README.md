...
* **Controls:** `Refresh Now` (dashboard `117` only), `Sync Time` (Evb019 only; uses Home Assistant local time and the official app's Dashboard command), and guarded `Regenerate Now` / `Next Regeneration Step` (Evb019 only, refreshes state before send so only valid action is available). Regen cycle timings are intentionally not writable. The integration checks clock drift every six hours and synchronizes automatically only when it exceeds five minutes.
